function compute() {
  ...
  var dominance = power / global_power / 1000 * 100 / 3; // hybrid
...
$(function() {
  ...
  $.getJSON('https://server.webdollarminingpool.com', function(data) {
    var hashrate = parseInt(data.networkHashRate / 1000 / 1000); // in Mh/s

    if (hashrate.length <= 5) {
      $('#global_power_input').val(hashrate);
    }
